...
\begin{frame}{System Architecture}
  \begin{columns}[t]
    \column{0.45\textwidth}
      \textbf{Modular OOP Design}
      \begin{itemize}
        \item \texttt{src/app.py} -- Streamlit UI and user input handling
        \item \texttt{src/data\_loader.py} -- Data cleaning and preprocessing
        \item \texttt{src/models.py} -- ML model pipelines and leakage guards
        \item \texttt{src/recommender.py} -- Filtering, ranking, and recommendation logic
        \item \texttt{src/evaluate\_model.py} -- dedicated evaluation script
        \item \texttt{src/utils.py} -- train-only budget bin creation
      \end{itemize}
    \column{0.52\textwidth}
      \textbf{Two-Stage Recommendation Flow}
      \vspace{0.4em}

      \begin{tikzpicture}[
        node distance=0.45cm,
        box/.style={rectangle, rounded corners=3pt, draw=sctceblue,
                    fill=lightgray, text width=5.8cm,
        ...
        \node[box, below=of ui] (map)  {Lifestyle $\rightarrow$ Spec Mapping};
        \node[box, below=of map] (rf)   {Stage 1: RF Budget Classifier};
        \node[box, below=of rf]  (pool) {Filtered Candidate Pool};
        \node[box, below=of pool](sim)  {Stage 2: Similarity-Based Ranking};
        \node[box, below=of sim, fill=accentblue!20, draw=accentblue]
              (out)  {Top-5 Recommendations + Reasons};
        \draw[arrow] (ui) -- (map);
        \draw[arrow] (map) -- (rf);
        \draw[arrow] (rf) -- (pool);
        \draw[arrow] (pool) -- (sim);
...
\begin{frame}{Methodology -- Lifestyle Mapping Detail}
  \begin{columns}[t]
    ...
    \column{0.48\textwidth}
      ...
      \end{block}
      \vspace{0.5em}
      \begin{alertblock}{Why Similarity Ranking?}
        \small The match score ranks cars by preference alignment after the ML classifier narrows the candidate pool.
      \end{alertblock}
  \end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{System Working Flow}
  \centering
  \begin{tikzpicture}[
    node distance=0.55cm,
    box/.style={rectangle, rounded corners=4pt, draw=sctceblue,
                fill=lightgray, text width=7.2cm,
                minimum height=0.7cm, align=center, font=\small\bfseries},
    highlight/.style={rectangle, rounded corners=4pt, draw=accentblue,
                fill=accentblue!20, text width=7.2cm,
                minimum height=0.7cm, align=center, font=\small\bfseries},
    arrow/.style={-Stealth, thick, color=sctceblue, line width=1.2pt}
  ]
    \node[box]       (inp)  {User Inputs (Budget + Lifestyle Preferences)};
    \node[box, below=of inp]  (map)  {Lifestyle to Spec Mapping};
    \node[box, below=of map]  (rf)   {Random Forest Budget Classification};
    \node[box, below=of rf]   (fil)  {Filter Vehicles by Predicted Budget Tier};
    \node[box, below=of fil]  (sim)  {Similarity-Based Ranking};
    \node[highlight, below=of sim] (out)  {Top-5 Recommended Vehicles + Explanation Badges};
    \draw[arrow] (inp) -- (map);
    \draw[arrow] (map) -- (rf);
    \draw[arrow] (rf)  -- (fil);
    \draw[arrow] (fil) -- (sim);
  ...
  \begin{columns}[t]
    ...
    \column{0.49\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[box, below=of raw2] (bins2) {Budget Classes for App Data};
        \node[box, below=of bins2] (fit2) {Production Fit on All Records};
...
\begin{frame}{Technology Stack}
  \begin{center}
    ...
    \begin{tabularx}{0.95\textwidth}{>{\bfseries}lX}
      \toprule
      ...
      Preprocessing           & Scikit-Learn \texttt{ColumnTransformer}, \texttt{MinMaxScaler}, \texttt{SimpleImputer}, \texttt{OneHotEncoder} \\
...
\begin{frame}{Results -- Clean Evaluation}
  \begin{columns}[t]
    ...
    \column{0.51\textwidth}
      \begin{block}{System Capabilities Achieved}
        \begin{itemize}
          \setlength\itemsep{0.35em}
          \item Lifestyle-to-specification translation from budget, usage, fuel, body type, and priority inputs
          \item Random Forest budget classification with \textbf{90.91\% clean holdout accuracy}
          \item Stratified 5-fold validation with \textbf{92.54\% average RF accuracy}
          \item Top-5 personalized recommendations ranked by preference match score
          \item CART Decision Tree retained for interpretable model comparison
          \item Robust CSV handling using regex extraction, numeric conversion, imputation, and leakage guards
        \end{itemize}
      \end{block}
      \vspace{0.4em}
      \begin{block}{Evaluation Output Includes}
        RF accuracy, CART accuracy, dummy baseline, classification report,
        confusion matrix, 5-fold CV score, and top feature importances.
      \end{block}
  \end{columns}
\end{frame}

\begin{frame}{Results -- Model Interpretation}
  \begin{columns}[t]
    ...
    \column{0.50\textwidth}
      \begin{block}{System Capabilities Achieved}
        \begin{itemize}
          ...
          \item Clean Random Forest budget classification with \textbf{90.91\% holdout accuracy}
          \item Robust CV validation with \textbf{92.54\% average RF accuracy}
          \item Top-5 personalized recommendations ranked by preference similarity
          \item Explicit leakage prevention through feature guards and tests
          \item Saved model support using \texttt{joblib} for faster app startup
          \item Modular architecture for future model or data source extension
        \end{itemize}
      \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \item The corrected evaluation protocol makes reported accuracy more trustworthy:
          no full-data prediction is treated as test accuracy.
    ...
    \item The final app can still train on all available data for recommendations after
          clean evaluation has been completed separately.
  \end{itemize}
  \vspace{0.5em}
